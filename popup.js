// AIzaSyCKAc7rqu7YhvQAWhIl9mASRkTdF-VmcGI

// popup.js

document.addEventListener("DOMContentLoaded", async () => {
    const outputDiv = document.getElementById("output");
    const API_KEY = 'AIzaSyCKAc7rqu7YhvQAWhIl9mASRkTdF-VmcGI';  
    const API_URL = 'http://yt-lb2-1764511110.ap-southeast-2.elb.amazonaws.com:80';
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      ...
      if (match && match[1]) {
        ...
        if (predictions) {
          ...
          outputDiv.innerHTML += `<p>Fetched ${comments.length} comments. Generating summary...</p>`;

          const summary = await summarize(comments.map(comment => comment.text));

          if (summary) {
            outputDiv.innerHTML += `
              <div class="section">
                <div class="section-title">Summary of Comments</div>
                <p>${summary}</p>
              </div>
              `;
            } else {
              outputDiv.innerHTML += "<p>Failed to generate summary.</p>";
            }
    ...
    async function summarize(comments) {
      try {
        ...
        const data = await response.json();
        if (response.ok) {
          return data.summary; // The result now includes sentiment and timestamp
        } else {
          throw new Error(result.error || 'Error fetching predictions');
        }
      
      } catch (error) {
        console.error("Error featching summary of comments:", error);
        outputDiv.innerHTML += "<p>Error fetching summary of comments.</p>";
      }
    }
  });
